...
?>
<div class="task-list">
    <h2>Actividades Para Mantenimiento de Equipos Estacionarios</h2>
    <a href="index.php?action=create" class="btn">Nueva Tarea</a>
    <ul>
        <?php foreach ($tasks as $task): ?>
            <li class="<?= (!empty($task['is_completed']) && $task['is_completed']) ? 'completed' : '' ?>">
                <span><?= htmlspecialchars($task['title'] ?? '', ENT_QUOTES, 'UTF-8') ?></span>
                <span><?= htmlspecialchars($task['description'] ?? '', ENT_QUOTES, 'UTF-8') ?></span>
                <div>
